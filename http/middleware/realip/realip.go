...
var (
	// DefaultForwardHeaderKey 协商forward ip 的 hander key名称
...
func NewDefault() router.Middleware {
	return New(&Options{
		forwardHeaderKeys: defaultScanForwardHeaderKey,
	})
}
...
func New(options *Options) router.Middleware {
	r := new(realip)
	r.addScanKey(options.forwardHeaderKeys)
	return r
}
...
//Handler RealIP is a middleware that sets a http.Request's RemoteAddr to the results
