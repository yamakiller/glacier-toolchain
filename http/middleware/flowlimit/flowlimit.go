...
// NewCookieKeyModeLimiter 实例化-根据cookie中特定的Key进行限制
...
func new(rate float64, capacity int64, mode Mode) *Limiter {
	...
	return &Limiter{
		rate:              rate,
		capacity:          capacity,
		limiters:          make(map[string]flowcontrol.FlowLimiter),
		l:                 zap.Instance().Named("Flow Limiter"),
		mode:              mode,
...
type Limiter struct {
	rate              float64
	capacity          int64
	limiters          map[string]flowcontrol.FlowLimiter
	count             int64
	maxSize           int64
	mu                sync.RWMutex
	l                 logger.Logger
...
// GetLimiter 获取对应的限制器
func (l *Limiter) GetLimiter(r *http.Request) flowcontrol.FlowLimiter {
	id := l.getLimiterID(r)
...
func (l *Limiter) addLimiter(id string) flowcontrol.FlowLimiter {
	l.mu.Lock()
	defer l.mu.Unlock()
